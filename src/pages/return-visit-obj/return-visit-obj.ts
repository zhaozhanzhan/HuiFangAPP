...
@IonicPage()
@Component({
  selector: "page-return-visit-obj",
  templateUrl: "return-visit-obj.html"
})
export class ReturnVisitObjPage {
  ...
  public surplusNum: number = 0; // 本期剩余回访用户数
  ...
  ionViewDidLoad() {
    console.log("ionViewDidLoad ReturnVisitObjPage");
    ...
    this.sendData.sorting = this.sortOrder; // 排序方式 按街道排 street,按居委排 residence,按地址排 address
    // 请求列表数据
    this.reqData(
      this.reqUrl,
      this.sendData,
    ...
    this.httpReq.get(url, reqObj, (data: any) => {
      ...
      if (data["data"] && data["data"]["result"] == 0) {
        ...
        if (data["data"] && _.isArray(data["data"]["visitObjectPage"])) {
          this.surplusNum = data["data"]["count"];
